...
const QuestionCard = (props) => {
  ...
  useEffect(()=>{
   ...
   for(let i=0;i<props.questionSets.length;i++){
     temp.push(comment);
     switchResponse.push(switchRes);
   }
   setCommentsResponse(temp);
   setSwitchsResponse(switchResponse);
   setInitialCommentsResponse(temp);
   setSwitchsInitialResponse(switchResponse);
  },[props])

  const handleChange = (id, newObject) => {
    let commentTemp=[...commentsResponse]
    temp[id] = newObject;
    commentTemp=[...temp]
    setCommentsResponse(commentTemp);
  };

  const handleSwitchChange = (e, id, value) => {
    let temp=[...switchResponse]
    switchResponse[id] = value;
    setSwitchsResponse(temp);
  ...
  const disableSubmit = () => {
    ...
    if(fileChoose.length>0 && noOfYesResponse==switchResponse.length){
      return false;
    }else if(noOfCommentsResponse>0 && (noOfYesResponse+noOfCommentsResponse)==switchResponse.length){
      return false;
    }else{
      return true;
    }
  ...
  const submitQuestions=()=>{
    ...
    for(let i=0;i<switchResponse.length;i++){
      payloadChunk={
        ...
        "response_no": props.selectedMapping.country_id+props.selectedMapping.control_id+props.task_no,
          ...
          {props.questionSets &&
            props.questionSets.map((q) => (
              ...
              >
                ...
                {switchResponse && !switchResponse[q.question_id-1] && <input
                    id="standard-basic"
                    placeholder="Comment"
                    variant="outlined"
                    style={{"width":"100%","border":"solid red 1px"}}
